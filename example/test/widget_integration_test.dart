...
void main() async {
  ...
  try {
    ...
    final user = await authService.login(UserModel(
      email: email,
      password: password,
    ));
    ...
    final sortedTypes = componentTypes.entries.toList()
    ...
    final supportedTypes = [
      ...
      'checkbox', 'radio', 'select', 'selectboxes', 'button',
      'datetime', 'day', 'time', 'currency', 'survey', 'signature',
      'hidden', 'container', 'datamap', 'datagrid', 'editgrid',
      'panel', 'columns', 'htmlelement', 'content', 'fieldset', 'table', 'tabs', 'well',
      'file', 'nestedform', 'captcha',
      'custom'
    ];
    ...
    final testForm = forms.firstWhere(
      (f) => f.components.isNotEmpty,
      orElse: () => forms.first,
    );
    ...
    final submissionTestForm = forms.firstWhere(
      (f) => f.components.isNotEmpty && f.components.length < 10,
      orElse: () => testForm,
    );
    ...
    try {
      ...
      final submission = await submissionService.submit(
        submissionTestForm.path,
        submissionData,
      );
